...
namespace TARE::OpenGL {
	...
	namespace Utils {
		...
		static GLenum TextureTarget(bool multisampled, bool array = false) {
			if (multisampled) return array ? GL_TEXTURE_2D_MULTISAMPLE_ARRAY : GL_TEXTURE_2D_MULTISAMPLE;
			else return array ? GL_TEXTURE_2D_ARRAY : GL_TEXTURE_2D;
		}
	...
	void OpenGL_Framebuffer::ClearAttachment(uint attachment, int value)
	{
	...
	{
		CHECKF(index < _ColorAttachments.size(), "Index out of bounds for color attachments!");
